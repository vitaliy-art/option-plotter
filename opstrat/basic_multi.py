...
def multi_plotter(
    ...
    """
    Plots a basic option payoff diagram for a multiple options and
    resultant payoff diagram
    ...
    op_list: list of dictionary
       ...
       Each dictionary must contain following keys
       ...
       'op_type': kind {'c','p'}, default:'c'
          Option type>> 'c': call option, 'p':put option
    ...
    for op in op_list:
        ...
        try:
            ...
            contract = 1
        y_list.append(
            payoff_calculator(x, op_type, strike, op_pr, tr_type, contract)
        )
    ...
    def plotter():
        ...
        plt.fill_between(
            x, y, 0,
            alpha=0.2, where=y > y0, facecolor='green', interpolate=True
        )

        plt.fill_between(
            x, y, 0,
            alpha=0.2, where=y < y0, facecolor='red', interpolate=True
        )
